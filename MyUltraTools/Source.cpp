...
float g_rotSpeed = 0.005f; 
float g_bgRed = 0.2f;
float g_bgGreen = 0.5f;
float g_bgBlue = 0.8f;
float rotX = 0.f;
float rotY = 0.f;
float rotZ = 0.f;
...
void FetchSceneParams() {
    ...
    while (true) {
		...
		if (res && res->status == 200) {
			...
			float X = findFloat("rot_x");
			float Y = findFloat("rot_y");
			float Z = findFloat("rot_z");
			{
				std::lock_guard<std::mutex> lock(g_paramsMutex);
				rotX = X;
				rotY = Y;
...
void DrawScene() {
	...
	D3DXCOLOR bgColor(g_bgRed, g_bgGreen, g_bgBlue, 0.0f);
...
void UpdateScene() {
	red += colormodr * g_bgRed;
	green += colormodg * g_bgGreen;
	blue += colormodb * g_bgBlue;
	...
	XMVECTOR rotaxis2 = XMVectorSet(0.1f * rotX, 0.1f * rotY, 0.1f * rotZ, 0.0f);
