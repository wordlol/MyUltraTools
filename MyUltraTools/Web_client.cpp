...
// ---- Параметры, которые будут приходить из бэкенда ----
std::mutex paramsMutex;
float g_rotSpeed = 0.0005f;   // скорость вращения
float g_bgRed = 0.0f;
float g_bgGreen = 0.0f;
float g_bgBlue = 0.0f;
bool g_useWireframe = false;   // можно добавить переключение, но пока не используем

// Функция фонового опроса параметров
void FetchSceneParams() {
    httplib::Client cli("http://localhost:8000");
    while (true) {
        auto res = cli.Get("/scene/params");
        if (res && res->status == 200) {
            std::string body = res->body;
            // Примитивный парсинг JSON без библиотек
            ...
            float speed = findFloat("rot_speed");
            float r = findFloat("bg_red");
            float g = findFloat("bg_green");
            float b = findFloat("bg_blue");
            {
...
{
    VertexTex() {};
    VertexTex(float x, float y, float z, float u, float v) : pos(x, y, z), texCoord(u, v) {}
    XMFLOAT3 pos;
    XMFLOAT2 texCoord;
};
...
void DrawScene() {
    // Получаем текущие параметры из бэкенда (безопасно)
    float currentRotSpeed, currentRed, currentGreen, currentBlue;
    {
        std::lock_guard<std::mutex> lock(paramsMutex);
        currentRotSpeed = g_rotSpeed;
        currentRed = g_bgRed;
        currentGreen = g_bgGreen;
        currentBlue = g_bgBlue;
    }
    // Устанавливаем цвет фона (очистка)
    D3DXCOLOR bgColor(currentRed, currentGreen, currentBlue, 0.0f);
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE rhinstance, LPSTR lpstr, int nshowcmd)
{
    ...
    // Запускаем фоновый поток для получения параметров из бэкенда
    std::thread fetcher(FetchSceneParams);
    fetcher.detach();
